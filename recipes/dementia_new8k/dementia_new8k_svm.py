...
task = 'dementia_new8k'
feat_type = ['ivecs', 'mfcc', 0]  # provide the types of features, type of frame-level feats, and deltas to use e.g.: 'fisher', 'mfcc', 0
...
for ga in gaussians:
    ...
    for c in list_c:
        ...
        print("with", c, "-", ga, acc, " auc:", auc)
        util.results_to_csv(file_name='exp_results/results_{}.csv'.format(task), list_columns=['Exp. details', 'C value', 'Accuracy', 'AUC'],
                            list_values=[file_n, c, acc, auc])
    print()
    print(std_scaler)
    print()
