...
work_dir = '/home/jvel/PycharmProjects/the_speech/'  # for ubuntu (native bob kaldi)
# in and out dirs
audio_dir = '/media/jvel/data/audio/MS/'
...
# for training the extractor and extracting i-vecs when there exists already pretrained UBMs and pretrained ivec extractors
    ...
    for g in list_n_clusters:
        for deltas in [0, 1, 2]:
            ...
            for folder_name in list_sets:  # iterating over the list of sets where the features live
                ...
                list_extractors_files = util.traverse_dir_2(ubm_dir,
                                                            'ivec_mdl_{0}g_{1}{2}-{3}del_{4}.ivexc'.format(g, feats_info_ubm[0], feats_info_ubm[2], feats_info_ubm[1], 'demencia94ABC'))
                print(list_ubm_files)
                print(list_extractors_files)
                extract_ivecs.extract_ivecs_pret_ubm_ivexc(list_mfcc_files=list_mfcc_files,
                                                           list_extractors_files=list_extractors_files, g=g,
                                                           list_fubms=list_ubm_files,
                                                           mfcc_info=feats_info_ubm, recipe=recipe, out_dir=out_dir,
                                                           folder_name=folder_name)
