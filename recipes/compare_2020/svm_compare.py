...
task = 'mask'
feat_info = ['fisher', 'mfcc', 2, 40]  #  provide the types of features and frame-level features to use e.g.: 'fisher', 'mfcc', 'xvecs'
...
for gauss in gaussians:
    ...
    kernel = 'linear'
    scores = []
    print("with --> ", feat_info, gauss, "gauss", kernel)
    for c in list_c2:
        for g in list_gamma:  # [1367, 684531, 8754, 3215, 54, 3551, 63839845, 11538, 148111, 4310]:
            ...
            posteriors, clf = svm_fits.train_linearsvm_cpu(x_combined, y_combined.ravel(), c=c, X_eval=x_test, class_weight='balanced')
            ...
            uar = recall_score(y_test, y_pred, labels=[1, 0], average='macro')
            scores.append(uar)
            print("with", c, "-", g, uar)

    # Train SVM model on the whole training data with optimum complexity and get predictions on test data
    optimum_complexity = list_c2[np.argmax(scores)]
    print('\nOptimum complexity: {0:.6f}'.format(optimum_complexity))

    posteriors, clf2 = svm_fits.train_linearsvm_cpu(x_combined, y_combined.ravel(), X_eval=x_test, c=optimum_complexity, class_weight='balanced')
    preds = np.argmax(posteriors, axis=1)
    uar = recall_score(y_test, preds, labels=[1, 0], average='macro')
    print("test:", uar)
    print()
...
def predict(best_c):
    ...
    print("with", best_c, "-", g, recall_score(y_test, y_pred, labels=[1, 0], average='macro'))
