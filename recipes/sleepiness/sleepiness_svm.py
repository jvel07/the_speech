...
for ra in [3]:
    ...
    for srand in srand_list:
        ...
        x_train, x_dev, x_test, y_train, y_dev, y_test,  file_n = load_data_full(
                                                ...
                                                gauss='512dim-BEA_AUG_DNN-{0}'.format(srand),
                                                # gauss='{}g'.format(ga),
                                                task=task, feat_type=feat_type,
                                                n_feats='')
        ensemble_feats = False
        if ensemble_feats:
        ...
        csv_name = 'results_{}_srands_BEA_AUG.csv'.format(task)
        ...
        np.savetxt('preds_BEA_AUG_{}/best_preds_dev_{}_srand_{}.txt'.format(feat, 0.001, srand), dev_preds)
        ...
        util.results_to_csv(file_name='exp_results/{}'.format(csv_name),
                            ...
                            list_values=[os.path.basename(file_n), feat_type[2], optimum_complexity, coef_test,
                                         std_flag, 'TEST', srand])
        np.savetxt('preds_BEA_AUG_{}/preds_test_{}_srand_{}.txt'.format(feat, 0.001, srand), y_pred)
        #
