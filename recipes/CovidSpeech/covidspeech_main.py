...
def do_fishers():
    ...
    for deltas in [0, 1, 2]:
        # info-purpose parameters from the frame-level extracted features #
        feats_info = [23, deltas, 'mfcc']  # info of the features (n_features/dimension, deltas, cepstral_type=choose between mfcc or plp)
        obs = ''
        # Format is: "featureType_recipeName_nMFCCs_nDeltas.mfcc"
        list_files_ubm = [
            '../../data/CovidSpeech/train/flevel/mfcc_CovidSpeech_23_train_{}del.mfcc'.format(deltas),
            '../../data/CovidSpeech/dev/flevel/mfcc_CovidSpeech_23_dev_{}del.mfcc'.format(deltas)
                          ]
        for folder_name in list_sets:
            list_mfcc_files = util.traverse_dir_2(feature_dir + folder_name, '*{}_{}_{}del.{}'.format(feats_info[0],
                                                                                                    folder_name,
