...
def compute_fishers(list_n_clusters, list_mfcc_files, out_dir, list_files_ubm, recipe, folder_name, feats_info):
    ...
    list_feats = []
    for file_ubm in tqdm(list_files_ubm, desc="Loading UBM files", position=0):
    ...
    print("Shape of the frame-level feats for the UBM:", array_mfccs_ubm.shape)
    del list_feats, array_feats

    # print(list_feats[0])
    # print("Fisher-vecs will be extracted using {} number of Gaussians!".format(list_n_clusters))

    for g in tqdm(list_n_clusters, desc="Training GMM with {} Gaussians.".format(list_n_clusters), position=0):
        list_fishers = []
        means, covs, priors = do_gmm(array_mfccs_ubm, g)  # training GMM
        # print("means", means.shape, "covs", covs.shape, "priors", priors.shape)
        # print("means0", means.shape[0], "covs0", covs.shape[0], "priors0", priors.shape[0])

        # Files for storing the mean, covs and weights (priors) of the GMM
        file_means = out_dir + '/UBM/means_gmm_{}{}_{}del_{}g'.format(feats_info[0], feats_info[2],
                                                                               feats_info[1], str(g))
        file_covs = out_dir + '/UBM/covs_gmm_{}{}_{}del_{}g'.format(feats_info[0], feats_info[2],
                                                                             feats_info[1], str(g))
        file_priors = out_dir + '/UBM/priors_gmm_{}{}_{}del_{}g'.format(feats_info[0], feats_info[2],
                                                                                 feats_info[1], str(g))
        try:
        ...
        for file_name in (pbar2 := tqdm(list_mfcc_files, desc="Computing fisher vectors", position=0)):  # This list should contain the mfcc FILES within folder_name
            current_feats = np.load(file_name, allow_pickle=True)  # this list contains the mfccs per FILE
            pbar2.set_description("Computing fisher vectors for {}".format(file_name.split('/')[-1]))

            # for feat in list_feat:  # iterating over the mfccs
            fish = vlf.fisher.fisher(current_feats, means, covs, priors,
                                     square_root=True, normalized=True, improved=True)  # Extracting fishers from features
            list_fishers.append(fish)
        # Output file (fishers)
        obs = str(feats_info[1])  # getting number of deltas info
        file_fishers = out_dir + '/' + folder_name + '/fisher/fisher-{}{}-{}-{}g-{}.fisher'.format(
            str(feats_info[0]), feats_info[2], obs, g, folder_name)
        # util.save_pickle(file_fishers, list_fishers)  # save as pickle
        np_fishers = np.vstack(list_fishers)
        np.savetxt(file_fishers, np_fishers, fmt='%.7f')  # save as txt
        print(" {0} fishers saved to: {1}".format(np_fishers.shape, file_fishers))
