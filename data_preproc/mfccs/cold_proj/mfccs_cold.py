...
def mfcc_ivec_extr(rows):
    ...
    for item2 in audio_list_wav_ubm[-rows:]:
       # sample2 = pkg_resources.resource_filename(__name__, item2)
        data2 = bob.io.audio.reader(item2)
        # MFCC
...
def mfcc_ubm(rows):
    ...
    for item in audio_list_wav_ubm[:rows]:
        sample = pkg_resources.resource_filename(__name__, item)
        data = bob.io.audio.reader(sample)
        # MFCC
...
if __name__ == '__main__':
    ...
    set_ = 'train'
    observation = 'fulltmt'
    dir_wav_ubm = '/home/egasj/PycharmProjects/iVectorsBob/audio/timit/TRAIN' #'../audio/cold/cold.{}/'.format(set_)  # '../audio/wav-non-75-ubm/'
    dir_timit_train = '/home/egasj/PycharmProjects/iVectorsBob/audio/timit/TRAIN'  # for UBM
    dir_timit_test = '/home/egasj/PycharmProjects/iVectorsBob/audio/timit/TEST'  # for UBM
    audio_list_timit_train = util.traverse_dir(dir_timit_train, ".WAV")
    audio_list_timit_test = util.traverse_dir(dir_timit_test, ".WAV")
