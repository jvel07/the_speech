...
def get_xvecs(list_sets, dest_task):
    for i in list_sets:
        dataset = SPKID_Dataset('/media/jose/hk-data/PycharmProjects/the_speech/kaldi_python/exp_30mfcc/xvectors_demencia_94abc_pretrained_vx/xvector.scp')
        # dataset = SPKID_Dataset('/media/jose/hk-data/PycharmProjects/the_speech/kaldi_python/exp_20mfcc/xvectors_{0}/xvector.scp'.format(i))
        xvecs = []
        for j in range(len(dataset)):
        ...
        np.savetxt('../data/{0}/{1}/xvecs/xvecs-30mfcc-0del-{2}dim-pretrainedVoxCel-{3}.xvecs'.format(dest_task, dest_task, x.shape[1], dest_task), x)
        print(x.shape)
