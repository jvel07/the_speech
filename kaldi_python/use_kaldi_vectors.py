...
def get_xvecs(list_sets, dest_task):
    for i in list_sets:
        dataset = SPKID_Dataset('/media/jose/hk-data/PycharmProjects/the_speech/kaldi_python/exp_40mfcc/xvectors_demencia_94abc_bea16k_40mfcc_aug/xvector.scp')
        # dataset = SPKID_Dataset('/media/jose/hk-data/PycharmProjects/the_speech/kaldi_python/exp_40mfcc/xvectors_{0}/xvector.scp'.format(i))
        xvecs = []
        for j in range(len(dataset)):
            xvecs.append(dataset.__getitem__(j))
        x = np.vstack(xvecs)
        # np.savetxt('../data/{0}/{1}/xvecs-40mfcc-0del-{2}dim-BEA16kNoAug-{3}.xvecs'.format(dest_task, i, x.shape[1], i), x)
        np.savetxt('../data/{0}/{1}/xvecs/xvecs-40mfcc-0del-{2}dim-BEA16kAug-{3}.xvecs'.format(dest_task, dest_task, x.shape[1], dest_task), x)
        print(x.shape)
